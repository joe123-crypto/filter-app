...
export async function POST(req: Request) {
  try {
    ...
    const { object: filterDetails } = await generateObject({
      ...
      schema: z.object({
        name: z.string().describe('A short, catchy name for the filter (e.g., "Cosmic Dream", "Retro Arcade").'),
      ...
      prompt: `Generate a creative and unique image filter concept based on the theme: "${theme}".`,
    });

    // 2. Generate a preview image based on the generated description
    const imagePrompt = `A stunning, high-quality sample photograph that perfectly demonstrates a photo filter named "${filterDetails.name}". The style is: ${filterDetails.description}.`;
